...
const DietLogModal: React.FC<DietLogModalProps> = ({ isOpen, onClose, onComplete, t, selectedDate }) => {
  ...
  const handleSubmit = async () => {
    ...
    const timeOfDayKeyMap: Record<TimeOfDay, TranslationKey> = {
      'Breakfast': 'timeOfDayBreakfast',
      'Lunch': 'timeOfDayLunch',
      'Dinner': 'timeOfDayDinner',
      'After Dinner': 'timeOfDayAfterDinner',
    };
    
    let summary = `[${t('dietLogTitle')} - ${timestamp}]\n`;
    summary += `- ${t('foodDescriptionLabel').replace(':', '')}: ${foodDescription}\n`;
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={handleClose}>
      <div className="bg-zinc-800 rounded-lg shadow-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
        <h2 className="text-2xl font-bold text-amber-400 mb-2 text-center">{t('dietLogTitle')}</h2>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-1">{t('foodDescriptionLabel')}</label>
            <textarea value={foodDescription} onChange={e => setFoodDescription(e.target.value)} rows={2} placeholder={t('foodDescriptionPlaceholder')} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500"></textarea>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {(['Breakfast', 'Lunch', 'Dinner', 'After Dinner'] as TimeOfDay[]).map(tod => (
                <button key={tod} onClick={() => setTimeOfDay(tod)} className={`px-3 py-2 rounded-md text-sm transition-colors ${timeOfDay === tod ? 'bg-amber-600 text-white' : 'bg-zinc-700 hover:bg-zinc-600'}`}>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} placeholder={t('notesPlaceholder')} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500"></textarea>
          </div>
        ...
        <div className="mt-6 flex justify-end space-x-3">
          ...
          <button onClick={handleSubmit} className="px-4 py-2 bg-amber-600 text-white rounded-md hover:bg-amber-500 transition-colors">{t('completeCheckin')}</button>
        </div>
      </div>
    </div>
  );
